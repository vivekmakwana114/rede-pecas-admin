...
export function ImportPanel({ showToast }: { showToast: (msg: string, type?: 'success' | 'error' | 'info') => void }) {
  ...
  const handleImport = async () => {
    ...
    const result = await dispatch(importInventory({ items: parseResult.items }));
    if (importInventory.fulfilled.match(result)) {
      showToast(`Import complete — ${result.payload.inserted} added, ${result.payload.updated} updated.`, 'success');
